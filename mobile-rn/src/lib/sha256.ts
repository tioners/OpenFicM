...
class Sha256 {
  ...
  digestHex(): string {
    ...
    padding[end - 8] = high >>> 24;
    padding[end - 7] = high >>> 16;
    padding[end - 6] = high >>> 8;
    padding[end - 5] = high;
    padding[end - 4] = low >>> 24;
    padding[end - 3] = low >>> 16;
    padding[end - 2] = low >>> 8;
    padding[end - 1] = low;
    this.update(padding);
...
export async function sha256File(file: File, onProgress?: (bytesRead: number, totalBytes: number) => void): Promise<string> {
  if (!file.exists) throw new Error("要校验的文件不存在");
  ...
  try {
    while (completed < total) {
      const chunk = handle.readBytes(Math.min(1024 * 1024, total - completed));
  ...
  if (completed !== total) throw new Error(`文件读取不完整：${completed}/${total}`);
