...
- `standalone` 使用调试证书但关闭开发服务器模式并内置 JS，供手机脱离电脑手动测试；普通 `debug` 仍保留 Metro 开发体验。
...
- `style_sources` 保存参考书元数据，原文件和规范化正文位于应用私有目录；支持 TXT、Markdown、EPUB，并兼容 UTF-8、UTF-16 和 GB18030/GBK 文本。
...
### 2026-08-20 - 参考书库与作者文风闭环

文风功能改为独立参考书库，支持 TXT、Markdown、EPUB 导入、参考文风版本管理、助手与写作页选择；Agent 原稿和作者定稿建立本地关联，作者修改后可进化作品级文风。移除 Android 端服务地址与 FastAPI 路径，并限制 Lorn 运行时指令长度。
