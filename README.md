...
章节、角色、世界书之外还有一类内容：大纲、剧情走向、伏笔清单——它们**还没在正文里发生**。塞进世界书会被 Agent 当成既定设定，后续创作直接拿来用。

笔记按作品已有的结构分三级：整书放全书大纲和主线，卷放本卷走向和伏笔，章放这一章要达成什么。可以跨级移动——一条本来只影响单章的备忘，发现整卷都要用时提升为卷级即可。删除章节或卷时会问你笔记是保留还是一并删除，选保留就自动上浮到上一级。

Agent 能读写笔记，但提示词里只带标题，需要正文时才读取，所以笔记多了不会拖慢每次对话。
...
- 三级笔记（整书 / 卷 / 章），可跨级移动，删除容器时可选保留或一并删除
- 全文搜索 + 中文嵌入语义索引 + 重排精排
- 章节变化后的角色与世界书一致性检查
